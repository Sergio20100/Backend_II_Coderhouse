...
export const registerUser = async (req, res) => {
  ...
  try {
    ...
    const userFound = await userServices.getUserByEmail(newUser.email);
    if (!userFound) {
      try {
      ...
      res.status(400).json({ status: "Error", message: new ErrorManager("El Email del usuario ya existe", 400) });
    }
    // res.redirect('/login'); 
  } catch (error) {
